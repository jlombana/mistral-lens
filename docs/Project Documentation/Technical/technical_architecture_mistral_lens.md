...
| Topic Accuracy | Exact match (%) | > 80% | 73.3%¹ |
| Topic Score | LLM-as-judge (1-5) | > 4.0 | 4.5 |
...
The topic classifier uses a **fixed taxonomy prompt** with 17 predefined categories from the repliqa dataset. The model receives the document text (truncated to 2000 chars) and the full category list, and must return exactly one category name.

- **Zero-shot accuracy**: 73.3% exact match (11/15 correct)
- **LLM-judge semantic score**: 4.5/5 — misclassifications are all near-misses
- **Error pattern**: 4 errors all involve "News Stories" or "Local News" as ground truth, where the model selects a more specific but technically correct category (e.g., "Local Politics and Governance")
- **Fine-tuning prepared**: Training dataset (50 docs) and scripts ready for `open-mistral-nemo` fine-tuning, expected to push accuracy >90%
...
¹ 73.3% exact match using fixed taxonomy (17 categories, zero-shot). All 4 misclassifications are near-misses where the model selects a more specific subcategory. Fine-tuning expected to reach >90%.
